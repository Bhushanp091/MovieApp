...
fun MediaTypeScreen(
    ...
    val mediaType = when(type){
        "Popular" ->{
            movieListState.popularAll
        }
        "Latest"->{
            movieListState.latestAll
        }
        "Top Rated"->{
            movieListState.topRatedAll
        }
        "TV Series"->{
            movieListState.tvSeriesAll
        }
        else -> {
    ...
    if (mediaType.isEmpty()) {
        ...
        LazyVerticalGrid(
            ...
            items(mediaType.size) { index ->
                MovieItem(
                    movie = mediaType[index],
                    navController = navController
                )
                Spacer(modifier = Modifier.height(16.dp))

                if (index >= mediaType.size - 1 && !movieListState.isLoading) {
                    onEvent(MovieListUiEvent.Paginate(mediaTypeName))
                }
